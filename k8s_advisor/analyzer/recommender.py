...
def recommend_resources(workload: dict, has_prometheus: bool = True) -> ResourceRecommendation:
    ...
    if cpu_request == 0:
        ...
        if recommended_cpu_request > cpu_request:
            rationale_parts.append(
                f"CPU under-requested ({avg_cpu:.0f}m avg vs {cpu_request:.0f}m request), increase to {format_cpu(recommended_cpu_request)}"
            )
        else:
        ...
        if cpu_request <= CPU_REDUCTION_BASELINE_M:
            ...
            rationale_parts.append(
                f"CPU over-requested ({avg_cpu:.0f}m avg vs {cpu_request:.0f}m request), reduce to {format_cpu(recommended_cpu_request)}"
            )
    else:
    ...
    if cpu_throttle > 0 and has_prometheus:
        ...
        if cpu_p95 > 0 and cpu_max > 0:
            ...
            rationale_parts.append(
                f"CPU throttled ({cpu_throttle:.1f}%), increase limit to {format_cpu(recommended_cpu_limit)}"
            )
        else:
            recommended_cpu_limit = max(100, recommended_cpu_request * 2.0)
            rationale_parts.append(
                f"CPU throttled but no P95 data, increase limit to {format_cpu(recommended_cpu_limit)}"
            )
    ...
    if mem_request == 0:
        ...
        rationale_parts.append(
            f"Memory request not set, recommend {format_memory(recommended_mem_request)} (avg * 1.25)"
        )
        ...
        if recommended_mem_request > mem_request:
            rationale_parts.append(
                f"Memory under-requested ({avg_mem:.0f}Mi avg vs {mem_request:.0f}Mi request), increase to {format_memory(recommended_mem_request)}"
            )
        else:
        ...
        recommended_mem_request = max(MEM_MIN_RECOMMENDED_MI, avg_mem * 1.25)
        rationale_parts.append(
            f"Memory over-requested ({avg_mem:.0f}Mi avg vs {mem_request:.0f}Mi request), reduce to {format_memory(recommended_mem_request)}"
        )
    else:
        # Request is reasonable
        recommended_mem_request = mem_request
    ...
    if workload["oom_killed_count"] > 0:
        ...
        if mem_p95 > 0:
            recommended_mem_limit = max(recommended_mem_request * 1.5, mem_p95 * 1.3, mem_max * 1.2)
        else:
            recommended_mem_limit = recommended_mem_request * 2.0
        rationale_parts.append(
            f"OOM killed {workload['oom_killed_count']}x, increase limit to {format_memory(recommended_mem_limit)}"
        )
    elif mem_limit > 0 and avg_mem > mem_limit * 0.85:
        # Near limit
        recommended_mem_limit = max(mem_limit * 1.3, recommended_mem_request * 1.5)
        rationale_parts.append(
            f"Memory near limit ({avg_mem:.0f}Mi avg vs {mem_limit:.0f}Mi limit), increase to {format_memory(recommended_mem_limit)}"
        )
    ...
    requires_manual = (
        cpu_request == 0
        or mem_request == 0
        or workload["oom_killed_count"] > 0
        or (cpu_throttle > 0 and has_prometheus)
    )
